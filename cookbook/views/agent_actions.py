...
from cookbook.agent_api.pantry import AgentPantryInputError, inventory_queryset
...
from cookbook.models import Food, MealPlan, MealType, ShoppingList, ShoppingListEntry
...
class AgentShoppingListCollectionView(APIView):
    ...
    def get(self, request):
        return Response([shopping_list_payload(obj) for obj in ShoppingList.objects.filter(space=request.space).order_by('name', 'id')])
...
class AgentShoppingEntryDetailView(APIView):
    ...
    def patch(self, request, pk):
        ...
        if replay:
            return replay
        current = self._obj(request, pk)
        if current is None:
            return Response(status=status.HTTP_404_NOT_FOUND)
        before = shopping_entry_payload(current)
        try:
            with transaction.atomic():
                locked = ShoppingListEntry.objects.select_for_update().get(pk=pk, space=request.space)
                update_shopping_entry(request, locked, request.data)
                updated = self._obj(request, pk)
                response = shopping_entry_payload(updated)
                record_agent_event(request, action='shopping_entry.update', target_type='ShoppingListEntry', target_id=pk, before=before, after=response, response=response)
        except AgentHouseholdInputError as exc:
            return _error(exc, conflict='changed since' in str(exc).lower())
        except IntegrityError:
            return Response({'error': True, 'msg': 'The idempotency key conflicts with an existing write.'}, status=status.HTTP_409_CONFLICT)
...
class AgentMealTypeCollectionView(APIView):
    ...
    def get(self, request):
        return Response([meal_type_payload(obj) for obj in MealType.objects.filter(space=request.space).order_by('order', 'name')])
...
class AgentFdcSearchView(APIView):
    ...
    def get(self, request):
        try:
            return Response(search_foods(request.query_params.get('q') or request.query_params.get('query'), page_size=request.query_params.get('limit', 10)))
